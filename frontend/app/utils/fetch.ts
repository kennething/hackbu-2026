 ...
 * @param endpoint the endpoint to request. It will be automatically appended to the base URL, **so it SHOULD start with a `/`**.
...
async function requestEndpoint<T>(endpoint: string, method?: string, body?: object): Promise<T | void> {
  ...
  const res = await fetch(config.public.backendUrl + endpoint, options);
